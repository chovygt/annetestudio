...
const WIZARD_STEPS = [{ label: 'Pago' }, { label: 'Aplicar a facturas' }]
...
function emptyLine() {
  return { rowId: crypto.randomUUID(), compra_id: null, monto: null }
}

export default function PagosProveedoresSection({ onMessage }) {
  ...
  const refreshList = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(
        ...
        pagos_proveedor_aplicacion ( id, compra_id, monto_aplicado )
      `
      )
    ...
    setLoading(false)
    if (error) {
      onMessage?.({ ok: false, text: `No se pudieron cargar pagos: ${error.message}` })
      return
    }
    const list = (data || []).map((r) => {
      const apps = r.pagos_proveedor_aplicacion || []
  ...
  const loadSaldos = useCallback(async () => {
    const { data: v, error: ev } = await supabase
      .from('v_compras_saldo')
      .select('compra_id, saldo_pendiente, monto_total, monto_pagado, fecha_compra')
      .gt('saldo_pendiente', 0.005)
    if (ev) {
      onMessage?.({ ok: false, text: `Saldos de compras: ${ev.message}` })
      return
    }
    const list = (v || []).filter((x) => Number(x.saldo_pendiente) > 0.0001)
    setSaldos(list)
    const ids = list.map((s) => s.compra_id)
    if (ids.length === 0) {
      setComprasInfo(new Map())
      return
    }
    const { data: compras, error: ec } = await supabase
      .from('compras')
      .select('id, proveedores ( nombre )')
      .in('id', ids)
    if (ec) {
      onMessage?.({ ok: false, text: `Compras: ${ec.message}` })
      return
    }
    const m = new Map(
      (compras || []).map((c) => [c.id, { proveedor: c.proveedores?.nombre || '—' }])
    )
    setComprasInfo(m)
  }, [onMessage])
  ...
  const compraOptions = useMemo(() => {
    return saldos.map((s) => {
      const info = comprasInfo.get(s.compra_id)
      const prov = info?.proveedor || '—'
      const label = `${prov} · ${formatDate(s.fecha_compra)} · saldo ${formatMoneyGtq(s.saldo_pendiente)}`
      return {
        value: s.compra_id,
        label,
  ...
  const optionsByCompra = useMemo(() => new Map(compraOptions.map((o) => [o.compra_id, o])), [compraOptions])
  ...
  function validateStep0() {
    ...
    if (formaPagoRequiereCuentaBancaria(formaPago)) {
      ...
      if (!cuentaBancariaId) return 'Elige la cuenta bancaria de la que sale el retiro (obligatoria con transferencia o tarjeta).'
    }
    return ''
  }

  function validateStep1() {
    if (compraOptions.length === 0) return 'No hay compras con saldo pendiente.'
    const withData = lineas.filter((l) => l.compra_id && Number(l.monto) > 0)
    if (withData.length < 1) return 'Agrega al menos una factura y monto mayor a 0.'
    const ids = withData.map((l) => l.compra_id)
    if (new Set(ids).size !== ids.length) return 'No repitas la misma compra en dos líneas: une los montos en una sola.'

    for (const l of withData) {
      const max = optionsByCompra.get(l.compra_id)?.saldo
      if (max == null) return 'Compra no válida en el listado.'
  ...
  async function guardar() {
    ...
    try {
      ...
      if (comprobanteFile) {
        url = await uploadComprobanteToFacturasBucket(comprobanteFile, 'pagos_proveedor')
      }
      const withData = lineas.filter((l) => l.compra_id && Number(l.monto) > 0)
      ...
      const { data: pago, error: ep } = await supabase
        .from('pagos_proveedor')
        .insert({
          fecha_pago: toIsoDate(fechaPago),
        ...
        .single()
      if (ep) {
        if (ep.message?.includes('cuenta_bancaria') || ep.message?.includes('column')) {
          throw new Error(
            'Falta la columna `cuenta_bancaria_id` en `pagos_proveedor`. Ejecuta en Supabase `supabase/009_bancos_cuentas_movimientos.sql`.'
          )
        }
        if (ep.message?.includes('forma_pago')) {
          throw new Error(
            'Falta `forma_pago` en `pagos_proveedor`. Ejecuta `supabase/007_contado_pago_venta_y_proveedor.sql`.'
          )
        }
        throw ep
      }
      ...
      const { error: ea } = await supabase.from('pagos_proveedor_aplicacion').insert(apps)
      if (ea) {
      ...
      if (formaPagoRequiereCuentaBancaria(formaPago) && cuentaBancariaId) {
        const { error: em } = await insertMovimientoAutomaticoEgresoPago(supabase, {
          ...
          monto: montoTotal,
          fechaIso: toIsoDate(fechaPago),
        })
        if (em) {
          await supabase.from('pagos_proveedor').delete().eq('id', pago.id)
          if (em.message?.includes('movimientos_cuenta_bancaria') || em.message?.includes('column')) {
            throw new Error(
              'Falta la tabla `movimientos_cuenta_bancaria` o columnas. Ejecuta `supabase/009_bancos_cuentas_movimientos.sql`.'
            )
          }
          throw em
        }
  ...
  return (
    <div className="admin-panel">
      ...
      <p className="lead">
        ...
        compra con saldo pendiente. El monto por factura no puede superar el saldo.
      </p>
      <div className="admin-catalog-head">
        <Button
          ...
          onClick={refreshList}
        />
        <Button type="button" icon="pi pi-plus" label="Registrar pago" onClick={openWizard} />
      </div>
      ...
      <div className="admin-catalog-card">
        <DataTable
          ...
          emptyMessage="Aún no hay pagos a proveedores."
        >
          <Column field="fecha_pago" header="Fecha" body={(r) => formatDate(r.fecha_pago)} sortable />
          <Column
            header="Forma"
            style={{ width: '8.5rem' }}
            body={(r) => (r.forma_pago ? FORMA_PAGO_LABELS[r.forma_pago] || r.forma_pago : '—')}
          />
          <Column
            header="Cuenta"
          ...
          <Column
            ...
            sortable
          />
          <Column header="Facturas" body={(r) => r.numFacturas} style={{ width: '7rem' }} />
        ...
        <div className="admin-compra-wizard">
          ...
          {step === 0 ? (
            <div className="admin-compra-step-grid">
              <div>
                <label htmlFor="pp-fecha">Fecha del pago</label>
                <Calendar
                  id="pp-fecha"
                  value={fechaPago}
              ...
              <div>
                <label htmlFor="pp-forma">Forma de pago</label>
                <Dropdown
                  id="pp-forma"
                  value={formaPago}
              ...
              {formaPagoRequiereCuentaBancaria(formaPago) ? (
                <div className="admin-compra-field-full">
                  <label htmlFor="pp-cuenta">Cuenta bancaria (retiro) *</label>
                  <Dropdown
                    id="pp-cuenta"
                    ...
                    optionLabel="label"
                    optionValue="value"
                    placeholder="Selecciona la cuenta de donde sale el pago"
                    filter
                    className="w-full"
                    showClear={false}
                  />
                  <small className="admin-compra-help">
                    Obligatoria con transferencia o tarjeta: se registrará un egreso automático en
                    movimientos bancarios.
                  </small>
                </div>
              ) : null}
              <div className="admin-compra-field-full">
                <label htmlFor="pp-comentario">Comentario (opcional)</label>
                <InputTextarea
                  id="pp-comentario"
                  rows={3}
                  value={comentario}
              ...
              <div className="admin-compra-field-full">
                <label htmlFor="pp-url">URL de comprobante (opcional)</label>
                <InputText
                  id="pp-url"
            ...
            <div>
              {compraOptions.length === 0 ? (
                <p className="lead">No hay facturas con saldo pendiente.</p>
              ) : (
                <>
                  {lineas.map((line, idx) => (
                    <div key={line.rowId} className="admin-venta-line" style={{ flexWrap: 'wrap', gap: '0.5rem' }}>
                      ...
                      <Dropdown
                        value={line.compra_id}
                        options={compraOptions}
                        optionLabel="label"
                        optionValue="compra_id"
                        onChange={(e) => setLine(line.rowId, { compra_id: e.value, monto: null })}
                      ...
                      <InputNumber
                        ...
                        maxFractionDigits={2}
                        placeholder="Monto"
                        disabled={!line.compra_id}
                      />
                      {line.compra_id ? (
                        <small className="admin-compra-help">
                          Saldo: {formatMoneyGtq(optionsByCompra.get(line.compra_id)?.saldo)}
                        </small>
                      ) : null}
                      <Button
                        type="button"
                  ...
                  <div className="admin-compra-detalle-actions" style={{ marginTop: '0.75rem' }}>
                    <Button type="button" icon="pi pi-plus" label="Otra factura" onClick={addLinea} />
                  </div>
                </>
              )}
            </div>
          ...
          <div className="admin-catalog-dialog-actions">
            ...
            {step < 1 ? (
              ...
              <Button
                ...
                icon="pi pi-check"
                loading={saving}
                disabled={compraOptions.length === 0}
                onClick={guardar}
              />
            )}
          </div>
      ...
      <Dialog
        ...
        visible={Boolean(viewPago)}
        style={{ width: 'min(40rem, 96vw)' }}
        onHide={() => setViewPago(null)}
        modal
      >
        {viewPago ? (
          <div className="admin-compra-view">
            <p>
              <strong>Fecha:</strong> {formatDate(viewPago.fecha_pago)}
            </p>
            <p>
              <strong>Forma de pago:</strong>{' '}
              {viewPago.forma_pago ? FORMA_PAGO_LABELS[viewPago.forma_pago] || viewPago.forma_pago : '—'}
            </p>
            <p>
              <strong>Cuenta bancaria:</strong> {labelCuentaBancariaDesdePagoRow(viewPago)}
            </p>
            <p>
              <strong>Comentario:</strong> {viewPago.comentario || '—'}
            </p>
            {viewPago.comprobante_url ? (
              <p>
                <a href={viewPago.comprobante_url} target="_blank" rel="noreferrer" className="admin-link-inline">
                  Abrir comprobante
                </a>
              </p>
            ) : null}
            <DataTable value={viewPago.aplicaciones || []} dataKey="id" size="small" responsiveLayout="scroll">
              <Column field="compra_id" header="ID compra" />
              <Column header="Monto" body={(a) => formatMoneyGtq(a.monto_aplicado)} />
            </DataTable>
            <p className="admin-compra-total-inline">
              <strong>Total aplicado:</strong> {formatMoneyGtq(viewPago.montoTotal)}
            </p>
          </div>
        ) : null}
      </Dialog>
